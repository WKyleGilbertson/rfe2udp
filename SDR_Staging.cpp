    ...
    while (true)
    {
      ...
      if (_len > 0)
      {
        ...
        if (!_is_aligned)
        {
          ...
          {
            _is_aligned = true;
            printf("\r%60s\r", ""); // Clear the Hunt line
            printf("[+] LOCKED: Phase %u | Source: %.16s\n",
                   phase, hdr->dev_tag == 0 ? "Unknown" : &hdr->dev_tag);
          }
          else
            continue;
        }
  ...
  bool get_samples(uint8_t *_out, size_t _count)
  {
    while (true)
    {
      size_t _avail;
      {
        std::lock_guard<std::mutex> _lock(_mtx);
        if (_w_ptr >= _r_ptr)
          _avail = _w_ptr - _r_ptr;
        else
          _avail = R_SIZE - _r_ptr + _w_ptr;
      }
      if (_avail >= _count)
        break;
...
public:
  ...
  CorrRes process_block(const uint8_t *_data, size_t _count, double _freq)
  {
    ...
    for (size_t i = 0; i < _count; ++i)
    {
      ...
      // High Nibble (7-4) = Signs | Low Nibble (3-0) = Magnitudes
      // Bits: 7:Q1_s, 6:I1_s, 5:Q0_s, 4:I0_s | 3:Q1_m, 2:I1_m, 1:Q0_m, 0:I0_m

      // Sample 0
      double i0 = _map((b >> 0) & 1, (b >> 4) & 1); // I0_m=bit0, I0_s=bit4
      double q0 = _map((b >> 1) & 1, (b >> 5) & 1); // Q0_m=bit1, Q0_s=bit5

      // Sample 1
      double i1 = _map((b >> 2) & 1, (b >> 6) & 1); // I1_m=bit2, I1_s=bit6
      double q1 = _map((b >> 3) & 1, (b >> 7) & 1); // Q1_m=bit3, Q1_s=bit7
...
int main()
{
  ...
  while (true)
  {
    if (_rx.get_samples(_block.data(), _bytes_10ms))
    {
      ...
      blocks++;
      printf("\r[DSP] T+%5.2fs | I:%10.0f Q:%10.0f | Mag:%11.0f",
             blocks * 0.01, _res.i_val, _res.q_val, _mag);
      fflush(stdout);
    }
    // Small sleep to keep the CPU from spinning 100% while waiting for the ring buffer
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  return 0;
}